...
\part{Определения}
    ...
    \section{Гладкий путь, вектор скорости, носитель пути}
        ...
        \subsection{Вектор скорости}
            ...
            $\gamma'(t_0) = \left( \gamma'_1(t_0), \ \gamma'_2(t_0), \ \ldots, \ \gamma'_n(t_0) \right)$
    ...
    \section{Вариация функции на промежутке}
        ...
        При этом $f \in C^1 \left([a, b]\right)$ и $\mathrm{Var}^b_a f = \int\limits^b_a |f'(t)| dt$
    ...
    \section{Верхний и нижний пределы}
        ...
        \subsection{Верхний и нижний пределы}
            ...
            Нижний предел $x_n$ ~--- $\varliminf\limits_{n \rightarrow +\infty} x_n = \lim\limits_{n \rightarrow +\infty} \inf x_n := \lim x_n \in \mathbb{\overline{R}}$
\newpage
...
\part{Теоремы}
	...
	\section{Теорема Барроу}
		...
		\subsection{Теорема (Барроу)}
			...
			В условиях определения оказывается, что $\varphi$ ~--- диффиринцируема на $[a, b]$ и $\varphi'(x) = f(x)$ для любого $x \in [a, b]$
	...
	\section{Формула Ньютона-Лейбница, в том числе, для кусочно-непрерывных функций}
		...
		\subsection{Формулировка теоремы}
			...
			Пусть $f$ кусочно-непрерывна на $[a, b]$, $F$ ~--- первообразная $f$. 
	...
	\section{Теорема о формуле трапеций, формула Эйлера--Маклорена}
		...
		\subsection{Простейший случай формулы Эйлера-Маклорена}
			...
			$\int\limits^n_m f(x) \ dx = (\sum\limits^n_{i = m})^{\triangledown} f(i) - \frac{1}{2} \int^n_m f''(x) \left\{ x \right\} (1 - \left\{ x \right\}) \ dx$
    ...
    \section{Асимптотика степенных сумм}
        ...
        $1^p + 2^p + \ldots + n^p = \int\limits^n_a x^p dx + \dfrac{n^p + 1}{2} + \dfrac{1}{2} \int\limits^n_1 (x^p)'' \left\{ x \right\} (1 - \left\{ x \right\}) dx$
    ...
    \section{Формула Валлиса}
        ...
        \subsection{Формулировка}
            ...
            $\lim\limits_{n \rightarrow \infty} \left( \dfrac{(2n)!!}{(2n - 1)!!} \right)^2 \cdot \dfrac{1}{2n + 1} = \dfrac{\pi}{2}$
    ...
    \section{Теорема о вычислении аддитивной функции промежутка по плотности}
        ...
        \subsection{Формулировка}
            ...
            Пусть заданы $f$ и $\phi$, $f$ ~--- непрерывна, $\phi$ ~--- аддитивная функция промежутка, $f$ ~--- плотность $\phi$
    ...
    \section{Изопериметрическое неравенство}
        ...
        \subsection{Формулировка}
            ...
            $diam G < 1$ ($diam G = \sum\limits_{x, y \in G} \rho(x, y)$)
    ...
    \section{Неравенство Гёльдера для интегралов}
        ...
        \subsection{Формулировка}
            ...
            $\int\limits^b_a |fg| \leq (\int\limits^b_a |f|^p)^{\frac{1}{p}} (\int\limits^b_a |g|^q)^{\frac{1}{q}}$
        ...
        \subsection{Формулировка}
            ...
            \begin{enumerate}
                ...
                \item $t_n \rightarrow l > 0, l \in \mathbb{R}$ 
                    ...
                    $\varlimsup(x_n + t_n) = l \cdot \varlimsup x_n$
